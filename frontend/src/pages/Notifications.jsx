...
const Notifications = () => {
  ...
  const getTypeStyles = (type) => {
    switch (type) {
      ...
      case 'danger': return 'border-rose-500/20 bg-rose-500/5';
  ...
  return (
    <div className="flex min-h-screen bg-dark overflow-hidden">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden">
        {/* Header Section */}
        <header className="p-8 border-b border-white/5 bg-secondary/30 backdrop-blur-md">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 max-w-5xl mx-auto">
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                <h1 className="text-3xl font-black text-white tracking-tighter">Mes Messages</h1>
              </div>
              <p className="text-slate-400 text-sm">Consultez vos notifications et le suivi de vos activités foncières.</p>
            </div>
            ...
            <div className="flex items-center gap-2 p-1 bg-white/5 rounded-xl border border-white/10">
              <button 
                ...
                className={`px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wider transition-all ${
                  filter === 'all' ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-slate-400 hover:text-white'
                }`}
              >
                Tout voir
              </button>
              <button 
                ...
                className={`px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wider transition-all ${
                  filter === 'unread' ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-slate-400 hover:text-white'
                }`}
              >
            ...
            {loading ? (
              ...
              >
                <div className="w-20 h-20 bg-slate-800/50 rounded-full flex items-center justify-center mx-auto mb-6">
                  <MailOpen size={40} className="text-slate-600" />
                </div>
                <h3 className="text-xl font-bold text-white mb-2">Aucun message pour l'instant</h3>
                ...
                <AnimatePresence mode="popLayout">
                  {filteredNotifications.map((notif) => (
                    <motion.div
                      ...
                      } ${!notif.is_read ? 'shadow-lg shadow-black/20 ring-1 ring-white/10' : 'opacity-60'}`}
                    >
                      <div className="flex gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between gap-4 mb-1">
                            <h4 className={`font-bold text-lg leading-tight ${!notif.is_read ? 'text-white' : 'text-slate-300'}`}>
                              {notif.title}
                            </h4>
                            {!notif.is_read && (
                              <button 
                                onClick={() => markAsRead(notif.id)}
                                className="opacity-0 group-hover:opacity-100 p-2 bg-white/5 hover:bg-white/10 rounded-lg transition-all text-emerald-400"
